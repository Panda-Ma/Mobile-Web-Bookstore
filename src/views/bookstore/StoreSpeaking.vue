...
// speak => updateText(this.paragraph) =>play => createVoice(this.paragraph)
...
export default {
  ...
  data() {
    return {
      ...
      audio: null,
      audioCanPlay: false,
      currentPlayingTime: 0,//这里两个属性通过绑定audio的canplay事件获取
      totalPlayingTime: 0,
  ...
  methods: {
    // 在线语音合成
    createVoice(text) {
      //苹果手机和苹果电脑上audio组件必须采用同步方式，但axois采用异步，为了提高兼容性，采用xmlhttp
